...
def main():
    ...
    rrm_prots = get_RRM_prots(pfam_rrm_accs)
    ...
    h = open(dataset[0]+dataset[-1] + '_proteins_with_Combined_' + dataset + '_Above_70.tsv')
    ...
    for line in h:
        prot, is_rbp, comp_thresholds, seqs, boundaries = line.rstrip().split('\t')
        ...
        is_gerstberger_rbp, is_goterm_rbp, has_rrm, any_rbp, is_in_lc_prots = run_dataset_crosschecks(prot, lc_prots, gerstberger_rbps, goterm_rbps, rrm_prots)
...
def map_PTMs_and_SRdipeps(uniprot, domains, bounds):
    ...
    for bound in bounds:
        ...
        start_ind = int(start)
        end_ind = int(end)
        mods_hits = get_SRdom_mods(uniprot, start_ind, end_ind, mod_hits)
...
def get_SRdom_mods(prot, start, end, mod_hits):
...
def get_RRM_prots(pfam_rrm_accs):
