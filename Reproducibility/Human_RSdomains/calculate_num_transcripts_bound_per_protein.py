...
def plot_numtranscriptsbound(plotting_df):
    ...
    sns.boxplot(x='Category', y='Number of Transcripts Bound', data=temp_df, showfliers=False, palette=['0.8'], order=['Known', 'New', 'Non-SR-related'], width=1)
    ...
    plt.xticks([i for i in range(len(labels))], labels=[' ', ' ', ' '], fontname='Arial', fontsize=12)  # PRODUCES BLANK X-TICK LABELS, WHICH MAKES FIGURE SIZE AND SPACING CONSISTENT ACROSS FIGURE PANELS
    [t.set_color('white') for t in ax.xaxis.get_ticklines()]    # PRODUCES BLANK X-TICK LABELS, WHICH MAKES FIGURE SIZE AND SPACING CONSISTENT ACROSS FIGURE PANELS
    plt.yticks([x for x in range(0, 60000, 10000)], labels=['0', '10k', '20k', '30k', '40k', '50k'], fontname='Arial', fontsize=12)
    plt.xlabel(' ', fontname='Arial', fontsize=14)  # PRODUCES BLANK X-AXIS LABEL, WHICH MAKES FIGURE SIZE AND SPACING CONSISTENT ACROSS FIGURE PANELS
    ...
    plt.savefig('Fig 4H - NumberOfTranscriptsBound_v2.tiff', bbox_inches='tight', dpi=600)
    plt.close()
