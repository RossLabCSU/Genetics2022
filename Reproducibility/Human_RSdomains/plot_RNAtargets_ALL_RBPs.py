...
def main():
    ...
    is_known_df, common_to_uniprot, gerst_prots = get_known_srprots()
...
def barplot(df):
    ...
    plt.savefig('Fig4D - RNA target_Barplot_GerstbergerSet_Known_vs_New_vs_AllRBPs_CORRECTED.tiff', bbox_inches='tight', dpi=600)
    plt.close()
...
def get_known_srprots():
    ...
    sr_rbps, gerst_prots = get_sr_RBPs()
    ...
    return df, common_to_uniprot, gerst_prots
...
def get_sr_RBPs():
    ...
    for line in h:
        ...
        if is_gerst == '1':
            gerst_prots.append(items[0])
            
        prots.append(items[0])
        
    h.close()
    
    return prots, gerst_prots
