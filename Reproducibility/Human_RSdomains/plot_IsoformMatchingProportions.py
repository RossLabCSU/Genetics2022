...
def main():
    ...
    proportion_isos = get_num_isos_with_RS()
    df = {'Category':[],
            'Protein':[],
            'Proportion':[]
            }
...
def plot_nonmatch(df):
    ...
    plt.savefig('Fig 3B - ProportionIsoformsWithRSdomain_for_NonMatchProts.tiff', bbox_inches='tight', dpi=600)
    plt.close()
...
def plot_isos_with_perfect_match(df):
    ...
    plt.savefig('Fig 3C - ProportionIsoformsWithRSdomain_for_IsosWithPerfectRSmatch.tiff', bbox_inches='tight', dpi=600)
    plt.close()

        
def get_max_proportionMatching():
    
    h = open('TableS3_Human_RSdomains_IsoformComparison.tsv')
    header = h.readline()
    seqs_df = {}
    for line in h:
...
def get_num_isos_with_RS():
    ...
    for line in h:
        items = line.rstrip().split('\t')
        prot = items[1]
        isos_with_rs = len(items[7].split(','))
        total_isos = len(items[8].split(','))
        proportion = isos_with_rs / total_isos
        domains = items[12]
