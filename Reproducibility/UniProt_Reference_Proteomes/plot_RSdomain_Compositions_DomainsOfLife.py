...
def plotting(df):
    ...
    for aa_range in [(0,5), (5,10), (10,15), (15,20)]:
        ...
        sns.boxplot(x='Amino Acid', y='Percent Composition', data=small_df, hue='Domain of Life', palette=['0.8'], showfliers=False)
        ax.legend_.remove()
        sns.stripplot(x='Amino Acid', y='Percent Composition', data=small_df, hue='Domain of Life', palette=colors, size=1.5, dodge=True, jitter=0.2, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('Fig S7 - Full Composition Analysis of RS Domains - Domains of Life.tiff', bbox_inches ='tight', dpi=600)
    plt.close() 
