...
def plot_eukaryota(df, human_count):
    ...
    plt.ylabel('Number of Unique Genes Encoding\nProteins with RS domains', fontname='Arial', fontsize=14)
    fig = plt.gcf()
    fig.set_size_inches(2.5, 4)
    plt.savefig('Fig5C - Eukaryotes_NumberSRproteins.tiff', bbox_inches='tight', dpi=600)
    plt.close()
...
def plotting(df):
    ...
    plt.savefig('Fig5C - Non-Eukaryotes_NumberSRproteins.tiff', bbox_inches='tight', dpi=600)
    plt.close()
