...
def main(args):
    ...
    taxonid_to_organism = pickle.load(open('TaxonID_to_Organism_ReferenceProteomes.dat', 'rb'))
    taxonid_to_common_name = pickle.load(open('TaxonID_to_CommonOrganismName.dat', 'rb'))

    all_aas = 'ACDEFGHIKLMNPQRSTVWY'
    win_size, amino_acids, other_aas, comp_thresholds, disp_threshold, ign_disp, limited_aas, max_thresholds = get_params(args)
    combined_aas = ''.join(amino_acids)
    start_time = str(datetime.datetime.now())
    print('\nStart time:', str(datetime.datetime.now()), amino_acids, disp_threshold, '\n')
    ...
    frequency_output.write('\t'.join( ['File of Origin', 'Common Organism Name', 'Total Proteins', 'Total # of LCDs Idenitified', 'Raw Number of Proteins with LCD', 'Percentage of Proteins with LCD', 'ProtIDs with LCD'] ) + '\n')
    
    if args.verbose:
        output.write('\t'.join( ['File of Origin', 'Common Organism Name', 'Protein ID', '\t'.join([aa_group + '_Composition' for aa_group in amino_acids]), 'Combined Linear Dispersion'] ) + '\n')
    else:
        output.write('\t'.join( ['File of Origin', 'Common Organism Name', 'Protein ID','Domain Sequence','Domain Boundaries','Final Domain Composition','Final Domain Linear Dispersion','\t'.join([aa for aa in all_aas]) ]) + '\n')

    mins_df = {}
    maxs_df = {}
    
    for (dirname, dir, files) in os.walk('.'):
        num_fastas = sum( [1 if file.endswith('.fasta') else 0 for file in files] )
        completed = 0
        for file in files:
            if file.endswith('.fasta'):
                ...
                uniprot = file_info[0]
                uniprot_leader, taxonid, *remainder = uniprot.split('_')
                
                # MANUAL SKIP FOR FILES THAT YOU DON'T WANT TO ANALYZE
                if taxonid == 'SRproteinSequences' or taxonid == 'MultiProteome':
                    continue
                    
                if taxonid in taxonid_to_common_name:
                    common_name = taxonid_to_common_name[ taxonid ]
                else:
                    common_name = taxonid_to_organism[ taxonid ]
 
                h = open(file)

                proteome_size = get_proteome_size(file)
                total_seqs = 0
                ...
                for id, seq in fasta_parser(h):
                    ...
                    for i in range(len(seq) - win_size+1):
                        ...
                        limited_aa_comps, limited_group_comps = calc_composition(window, limited_aas, {aa_group:[] for aa_group in limited_aas})
                        
                        #MASK REGIONS THAT EXCEED THE MAXIMUM THRESHOLD FOR ANY SPECIFIED limited_aas
                        if 1 in [1 if limited_aa_comps[i] > max_thresholds[i] else 0 for i in range(len(limited_aas))]:
                    ...
                    if len(trimmed_boundaries) > 0 and args.verbose==False:
                        ...
                        junk, prot_id, *junk = id.split('|')
                        prots_with_lcd.add(prot_id)
                        for i in range(len(trimmed_seqs)):
                            output.write('\t'.join( [file, common_name, id, trimmed_seqs[i], trimmed_boundaries[i], str(round(final_comps[i], 2)), str(round(final_stddevs[i], 4)), '\t'.join([str(round(trimmed_seqs[i].count(aa) / len(trimmed_seqs[i]) * 100, 2)) for aa in all_aas]) ]) + '\n')
                    elif len(trimmed_boundaries) > 0 and args.verbose==True:
                        lcd_count += len(trimmed_seqs)
                        junk, prot_id, *junk = id.split('|')
                        prots_with_lcd.add(prot_id)
                        output.write('\t'.join( [file, common_name, id, '\t'.join( ['_'.join([str(round(x, 2)) for x in group_comps[aa_group]]) for aa_group in amino_acids] ), '_'.join([str(round(x, 4)) for x in normed_stddevs]) ] ) + '\n')
                    else:
                        continue
                        
                frequency_output.write('\t'.join( [file, common_name, str(proteome_size), str(lcd_count), str(len(prots_with_lcd)), str( len(prots_with_lcd) / proteome_size * 100 ), '_'.join(prots_with_lcd)] ) + '\n')
                completed += 1
                print(str(completed) + ' proteomes completed out of ' + str(num_fastas) + ' total proteomes\t', str(datetime.datetime.now()), 'S-Rcomp Thresholds:' + '_'.join([str(round(x)) for x in comp_thresholds]) )
...
def get_params( args ):
    ...
    if args.limited_residues:
        limited_aas = args.limited_residues.split('_')
        limited_aas = [x.upper() for x in limited_aas]
        max_thresholds = args.max_composition_thresholds.split('_')
        max_thresholds = [float(x)-0.000001 for x in max_thresholds]    # +/-0.000001 --> ADJUST VALUE FOR DOWNSTREAM FLOATING POINT COMPARISONS
    else:
        limited_aas = []
        max_thresholds = []
    ...
    return win_size, amino_acids, other_aas, comp_thresholds, disp_threshold, ign_disp, limited_aas, max_thresholds
...
def get_args(arguments):
    ...
    parser.add_argument('-x', '--limited_residues', type=str, default=None,
                        help="""Set of residues for which you would like to define maximum thresholds.
                        Residues can be grouped to set a maximum threshold for their combined
                        composition (similar to setting minimum thresholds). Residues or groups of residues
                        can also be underscore-separated to set distinct maximum thresholds.
                        This flag should be used in conjunction with the -m flag to set the thresholds.
                        
                        e.g.:
                        -x A -m 20  (alanine composition cannot exceed 20%)
                        -x AP -m 30     (combined alanine+proline composition cannot exceed 30%)
                        -x R_Q -m 15_25     (arginine composition cannot exceed 15% and glutamine content cannot exceed 25%)
                        -x AP_Q -m 30_25    (combined alanine+proline composition cannot exceed 30% and glutamine content cannot exceed 25%)""")
                        
    parser.add_argument('-m', '--max_composition_thresholds', type=str, default=None,
                        help="""Maximum percent composition threshold for each of the
                        residues specified using the -x flag.
                        
                        Sequence windows with a composition that exceeds the maximum threshold
                        for the specified residues are ignored by the algorithm and are
                        not considered LCDs. This enhances user control over the specificity
                        with which LCDs are identified.""")
